...
class SimpleCNN(nn.Module):
    def __init__(self, num_classes=100, poly=False, base_width=149):
...
def train():
    ...
    model1 = SimpleCNN(num_classes=100, poly=False, base_width=149).to(device)
    model2 = SimpleCNN(num_classes=100, poly=True, base_width=86).to(device)
    ...
    ax1.set_title('CIFAR-100 Training Accuracy')
    ax1.legend()
    ...
    ax2.set_title('CIFAR-100 Validation Accuracy')
    ax2.legend()
    ax2.grid(True, alpha=0.3)
    
    plot_path = './cifar100_training_plot.png'
